...
const { openMenu, resume, resetDrive, exitDrive, touchOn } = require('./drive-ui.cjs');
(async () => {
  ...
  try {
    ...
    await page.getByText('프롤로그 기록 · 해안 주행 클럽의 초대', { exact: true }).waitFor();
    ...
    await page.waitForTimeout(400); await touchOn(page);
