...
const roadDegreeByNode = new Map();
for (const road of CITY_ROADS) {
  roadDegreeByNode.set(road.a, (roadDegreeByNode.get(road.a) || 0) + 1);
  roadDegreeByNode.set(road.b, (roadDegreeByNode.get(road.b) || 0) + 1);
}
const maximumRoadDegree = Math.max(...roadDegreeByNode.values());
assert.ok(maximumRoadDegree <= 4, `City V2 교차로는 최대 4방향이어야 합니다. 현재 최대 ${maximumRoadDegree}방향입니다.`);
...
const routeStarts = [CITY_NODES.hub, CITY_NODES.eastCivic, CITY_NODES.northWest];
