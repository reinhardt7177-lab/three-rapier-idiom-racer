...
(async () => {
  ...
  try {
    ...
    await page.waitForTimeout(1200);
    const startButton = await page.getByRole('button', { name: '돌아오는 불빛 시작 →' }).boundingBox();
    assert.ok(startButton && startButton.height >= 48 && startButton.y + startButton.height <= 844, 'story entry is a reachable large touch target');
    assert.ok((await page.locator('.garage-story').boundingBox()).height <= 235, 'portrait story card does not grow across the car');
    ...
    await blocked.goto(BASE_URL, { waitUntil: 'networkidle' });
    ...
    await blocked.close();
    const externalRequests = requests.filter(url => !url.startsWith(BASE_URL) && !url.startsWith('data:'));
