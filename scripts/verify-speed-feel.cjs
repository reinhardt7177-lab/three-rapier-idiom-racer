...
const { openMenu, resume, exitDrive, touchOn } = require('./drive-ui.cjs');
(async () => {
  const output = path.resolve(process.env.GARAGE_ARTIFACT_ROOT || 'artifacts','speed-feel'); await fs.mkdir(output, { recursive: true });
  ...
  try {
    ...
    await openMenu(page); await page.getByRole('button', { name: '소리 켜기', exact: true }).click();
    await page.getByRole('button', { name: '소리 끄기', exact: true }).waitFor(); await resume(page); await page.waitForTimeout(500);
    ...
    await page.keyboard.up('KeyW'); await openMenu(page); await page.waitForTimeout(500);
    const pausedRms = await rms(); assert.ok(pausedRms < .000001);
    await resume(page);
    await page.keyboard.press('KeyR'); await page.waitForTimeout(300);
    await openMenu(page); await page.getByRole('slider', { name: '주행 소리 볼륨' }).fill('0'); await resume(page); await page.waitForTimeout(500);
    const zeroRms = await rms(); assert.ok(zeroRms < .000001);
    await openMenu(page); await page.getByRole('slider', { name: '주행 소리 볼륨' }).fill('60'); await resume(page); await page.waitForTimeout(500); assert.ok(await rms() > .00001);
    await openMenu(page); await page.getByRole('button', { name: '소리 끄기', exact: true }).click(); await resume(page); await page.waitForTimeout(500);
    const mutedRms = await rms(); assert.ok(mutedRms < .000001);
    await openMenu(page); await page.getByRole('button', { name: '소리 켜기', exact: true }).click(); await resume(page);
    ...
    await openMenu(page); assert.equal(await page.getByRole('button', { name: '소리 켜기', exact: true }).getAttribute('aria-pressed'), 'false');
