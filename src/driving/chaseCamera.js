...
export function createChaseCamera(camera) {
  ...
  return {
    update(position, forward, kmh, dt, { compact = false, snap = false, motion = true, push = 0, braking = 0 } = {}) {
      push = Math.min(1, Math.max(0, Number(push) || 0)); braking = Math.min(1, Math.max(0, Number(braking) || 0));
      const speedBlend = Math.min(1, Math.max(0, kmh) / 240);
      const distance = (compact ? 12 : 9.4) + (motion ? .4 * speedBlend : 0);
      target.copy(forward).multiplyScalar(-distance); target.y = (compact ? 4.4 : 3.3) + (motion ? .16 * braking - .12 * push : 0);
      ...
      camera.lookAt(aim);
      const fov = (compact ? 65 : 57) + (motion ? 6 * speedBlend + 1.8 * push - .7 * braking : 0);
